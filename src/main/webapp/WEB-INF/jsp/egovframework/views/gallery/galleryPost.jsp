			...
			<div class="card-body">
				...
				<form name="searchForm" id="search" method="GET">
	 				<div class="mb-3 span">
						<c:forEach items="${tagList}" var="tag">
							<a href="/gallery/galleryList.do?m_seq=${gPost.m_seq}&pageIndex=1&searchKey=tag&searchVal=${tag.tag_name}" style="text-decoration: none; color: #000; font-size: 12px;">
								&#35; 
				...
				<div class="mb-3">
					...
					<c:if test="${login.user_id != null}">
						<c:if test="${login.user_id == gPost.b_writer_id}">
				 			<button class="btn btn-outline-warning" onclick="location.href='/gallery/galleryModify.do?board_id=${gPost.board_id}&m_seq=${gPost.m_seq}'">수정하기</button>
							<button class="btn btn-outline-danger" onclick="location.href='/board/boardPostDe.do?board_id=${gPost.board_id}&m_seq=${gPost.m_seq}'">삭제하기</button>
						</c:if>
					</c:if>
			</div> 
	</div>
