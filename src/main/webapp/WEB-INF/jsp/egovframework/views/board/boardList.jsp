	...
	<main class="mt-5 pt-5">
		<div class="container-fluid px-4">
			...
			<c:if test="${login != null}">
				<div class="card-header seq" data-seq="${m_seq}">
					<a class="btn btn-primary float-end"
						href="javascript:openPop('/board/boardInsert.do?m_seq=${m_seq}', 'popup');">
					...
					<form action="/board/boardList.do" method="GET" class="row g-3">
						...
						<div class="col-auto">
							<input type="text" id="boardSearchVal" class="form-control" name="searchVal" value="${not empty bvo.searchVal ? bvo.searchVal:param.searchVal}"/>
						</div>
						<div class="col-auto">
							...
							<tbody>
								<c:forEach items="${noticeList}" var="notice">
									<tr style="background: #DFF6FF;">
										...
										<td>
											<a style="font-weight: 900; color: #182747;" href="javascript:openPop('/board/boardPost.do?board_id=${notice.board_id}&m_seq=${notice.m_seq}', 'popup');">${notice.b_title}</a>
								...
								<c:set var="num" value="${paging.totalRecordCount - ((pageIndex - 1)) * cntPerPage}" />
								<c:forEach items="${boardList}" var="board">
									<tr>
										<td><c:set var="num" value="${num - 1}" />${num + 1}</td>
										<td>
										...
										<c:choose>
											...
											<c:otherwise>
												<span class="category_deco">
													<c:if test="${not empty board.code_id}">
														<c:if test="${empty board.code_mid_id}">
															[${board.code_main_nm}]
														</c:if>
													</c:if>
													<c:if test="${not empty board.code_mid_id}">
														<c:if test="${empty board.code_sub_id}">
															[${board.code_mid_nm}]
														</c:if>
													</c:if>
													<c:if test="${not empty board.code_sub_id}">
														[${board.code_sub_nm}]
													</c:if>
												</span>
												<a href="javascript:openPop('/board/boardPost.do?board_id=${board.board_id}&m_seq=${board.m_seq}', 'popup');">${board.b_title}</a>
...
<script type="text/javascript">
		...
		var searchVal = $("#boardSearchVal").val();
	
		var cntPerPage = $("select[name=cntPerPage] > option:selected").val();

	$(document).ready(function() {

		
		console.log(searchKey, searchVal, cntPerPage)
...
/* 	
	...
	function linkPage(pageIndex) {
    	var m_seq = $('.seq').data("seq");
 		var searchKey = $("select[name=searchKey] > option:selected").val();
		var searchVal = $("#boardSearchVal").val();
		var cntPerPage = $("select[name=cntPerPage] > option:selected").val();
		
		location.href = "/board/boardList.do?m_seq=" + m_seq + "&pageIndex=" + pageIndex + "&cntPerPage=" + cntPerPage + "&searchKey=" + searchKey + "&searchVal=" + searchVal;
	}
