			...
			<ul>
				<c:forEach items="${questionList}" var="que" varStatus="stat">
						...
						<c:if test="${que.que_type eq 'W'}">
							<textarea class="area_con" id="area_${que.que_id}" maxlength="50"><c:forEach var="ans" items="${answerList}" varStatus="stat"><c:if test="${que.que_id == ans.que_id}">${ans.ans_con}</c:if></c:forEach></textarea>
						</c:if>
						<c:forEach items="${optionList}" var="option" varStatus="opt">
							<c:if test="${option.que_id == que.que_id}">
								<input type="checkbox" value="${option.option_id}" 
									<c:forEach var="a" items="${answerList}" varStatus="stat" >
									<c:if test="${a.option_id == option.option_id}">
										checked = checked
									</c:if>
									</c:forEach>
								class="checkbox <c:choose><c:when test="${option.option_type == 2}">two</c:when><c:when test="${option.option_type == 3}">none</c:when><c:when test="${option.option_type == 4}">etc</c:when><c:otherwise>alone</c:otherwise></c:choose>" id="op_${option.option_id}" data-que="que_${que.que_id}" />
								<label for="op_${option.option_id}">${option.option_nm}</label>
								<c:forEach var="a" items="${answerList}" varStatus="stat" >
									<c:if test="${not empty a.ans_etc_con and a.option_id == option.option_id}">
										<input class="etc_con" type="text" name="etc_con" value="${a.ans_etc_con}" />
									</c:if>
								</c:forEach>
							</c:if>
						</c:forEach>
...
<script type="text/javascript">
	...
	$(document).ready(function() {
		...
		changeLi(page_cnt);
 		...
 		$('.next').click(function() { // 다음 페이지로 넘어갈 때만
