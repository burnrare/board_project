			...
			<div class="card-body">
				...
				<c:if test="${login.user_id != null}">
					<c:if test="${login.user_id == boardPost.b_writer_id}">
						...
						<button class="btn btn-outline-danger" onclick="location.href='/board/boardPostDe.do?board_id=${boardPost.board_id}'">삭제하기</button>
					</c:if>
					<span style="float: right;">총 <c:out value="${boardPost.b_rcount}" /> 개의 댓글</span>
					<c:if test="${boardPost.b_type ne '1000'}">
						<button class="btn btn-outline-info" onclick="location.href='/board/boardRePost.do?board_id=${boardPost.board_id}'">답글 달기</button> 
					</c:if>
				</c:if>
			</div> 
	</div>
